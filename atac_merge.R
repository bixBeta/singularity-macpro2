...
if (length(arg)==0) {
  print(" Usage = Rscript atac_merge.R < PIN > ")  
...
fileNames <- list.files( dir, pattern = "ANNOTATED.txt$")
...
for (i in 1:length(fileNames)) {
  ...
  assign(strsplit(fileNames[i], "\\.")[[1]][1],
         value = read.delim(filePath[i], header = TRUE, row.names = NULL, sep = "\t", stringsAsFactors = F ),
         envir = .GlobalEnv)
}
...
new.data.frame = new.data.frame %>% select(1:4, "Strand","Annotation","Distance.to.TSS","Nearest.PromoterID","Entrez.ID","Nearest.Unigene","Gene.Name","Gene.Type", everything())
